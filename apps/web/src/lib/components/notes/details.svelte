...
<div
	class={cn(
		'hidden md:flex fixed right-0 h-[calc(100vh-4.5rem)] flex-col justify-start items-center bg-background overflow-y-auto transform transition-transform duration-300',
...
<!-- Mobile details panel (rendered inside drawer) -->
<div class="flex md:hidden flex-col w-full h-full bg-background overflow-y-auto">
	<!-- Metadata -->
	{#if activeNoteMetadataParams && tab === 'metadata'}
		<div class="flex flex-col gap-1.5 items-start w-full px-4 py-2.5 h-full overflow-auto">
			<div class="flex flex-row items-center justify-between w-full h-8 cursor-default">
		...
		<div class="w-full h-full overflow-auto">
			<div class="flex flex-col gap-1.5 items-start w-full h-full overflow-auto px-4 py-2.5">
				{#each tocItems as item}
					<button
						type="button"
						class="flex flex-row items-center justify-between w-full min-h-[32px] h-8 text-sm truncate font-normal text-muted-foreground hover:text-primary transition-all"
						style="padding-left: {item.indent}rem"
						on:click={() => {
							if (!nodeHeadings) return;
							$editor
